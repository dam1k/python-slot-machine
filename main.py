import random

MAX_LINES = 3
MIN_BET = 1
MAX_BET = 100

ROWS = 3
COLS = 3

symbol_count = {
     "A":2,
     "B":4,
     "C":6, 
     "D":8
}

symbol_value = {
    "A": 5,
    "B": 4,
    "C": 3,
    "D": 2
}

def deposit():
     while True: 
          amount = input('How much would you like to deposit? ')
          #isdigit() checks if the string contains only digits
          if amount.isdigit():
               amount = int(amount)
               if amount > 0:
                    break
               else:
                    print("Enter a value greater than 0")
          else:
               print("Enter a number")
     return amount

def get_number_of_lines():
     while True: 
          lines = input(f'Enter the number of lines to bet on (1-{str(MAX_LINES)})? ')
          #isdigit() checks if the string contains only digits
          if lines.isdigit():
               lines = int(lines)
               if 1 <= lines <= MAX_LINES:
                         break
               else:
                    print(f"Enter a value between 0 and {str(MAX_LINES)}")
          else:
               print("Enter a number")
     return lines

def get_bet():
     while True: 
          bet = input(f'Enter the amount you would like to bet ({MIN_BET}-{str(MAX_BET)})? ')
          #isdigit() checks if the string contains only digits
          if bet.isdigit():
               bet = int(bet)
               if MIN_BET <= bet <= MAX_BET:
                         break
               else:
                    print(f"Enter a value between {str(MIN_BET)} and {str(MAX_BET)}")
          else:
               print("Enter a number")
     return bet

def get_slot_machine_spin(rows, cols, symbols):
     all_symbols = []
     for symbol, symbol_count in symbols.items():
          for _ in range(symbol_count):
               all_symbols.append(symbol) 
     
     columns = []
     for col in range(cols):
          column = []
          current_symbols = all_symbols[:]
          for row in range(rows):
               value = random.choice(current_symbols)
               current_symbols.remove(value)
               column.append(value)
          columns.append(column)

     return columns


def print_slot_machine(columns):
     for row in range(len(columns[0])):
          for i, column in enumerate(columns):
               if i != len(columns):
                    print(column[row], end=" | ")
               else:
                    print(column[row], end="")
          print()

def check_winnnigs(columns, lines, bet, values):
     winnings = 0
     winning_lines = []
     for line in range(lines):
          symbol = columns[0][line] 
          for column in columns:
               symbol_to_check = column[line]
               for symbol_to_check in columns[line]:
                    if symbol_to_check != symbol:
                         break
               else: 
                    winnings += bet * values[symbol]
                    winning_lines.append(line + 1)
     return winnings, winning_lines

def spin(balance):

     lines = get_number_of_lines() 
     bet = get_bet()
          
     total_bet = lines * bet
          
     while True:
          if total_bet <= balance:
                print(f"You bet {total_bet}$")
                break
          else:
               print("You don't have enough money.")
     
          slots = get_slot_machine_spin(ROWS, COLS, symbol_count)
          print_slot_machine(slots)

          winnings, winning_lines = check_winnnigs(slots, lines, bet, symbol_count)
          print(f"You won ${winnings}")
          print(f"You won on lines {winning_lines}")
          return winnings

def main():
     balance = deposit()

     while True:
          print(f"Current balance is {balance}.")
          answer = input(f"Press enter to play (q to quit).")
     
          if answer == "q":
               break
          balance += spin(balance)

main()